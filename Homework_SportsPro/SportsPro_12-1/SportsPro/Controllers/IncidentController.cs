...
namespace SportsPro.Controllers
{
	public class IncidentController : Controller
	{
		...
		[Route("/incidents/list/unassigned-incidents")]
		public ViewResult UnassignedIncidents()
		{ 
			IncidentViewModel incidentViewModel = new IncidentViewModel()
			{ 
				Incidents = unitOfWork.Incidents.GetAllUnassignedIncidents().ToList()
			};

			return View("List", incidentViewModel);
		}

		[Route("/incidents/list/open-incidents")]
		public ViewResult OpenIncidents()
		{ 
			IncidentViewModel incidentViewModel = new IncidentViewModel()
			{ 
				Incidents = unitOfWork.Incidents.GetAllOpenIncidents().ToList()
			};
        ...
        public ViewResult UpdateIncidentsList(Technician technician)
        {
			...
			if (vm.Incidents.Count() == 0)
			{
				TempData["message"] = $"Technician {vm.Technician.Name} Has No Open Incidents.";
				vm.Technicians = unitOfWork.Technicians.GetAll().ToList();
				return View("TechniciansSelectList", vm);
			}
